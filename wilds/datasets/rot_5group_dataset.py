...
def make_environment(num, frac, train=True):
    ...
    Y4 = ((np.cos(3*angle))*X[:, 0] + (np.sin(3*angle))*X[:, 1] > 0).type(torch.float32)
...
class Rot5GroupDataset(WILDSDataset):
    def __init__(self, root_dir='data', download=False, split_scheme='official'):
        ...
        self._dataset_name = "rot_5group"
        self._data_dir = os.path.join(root_dir, self._dataset_name)

        train_data = make_environment(1000, [0.33, 0.33, 0.33, 0.005, 0.005], train=True)
